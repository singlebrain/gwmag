...
if ($logged=="yes") {$gname = $_SESSION[‘name’];} else { $gname = $_POST['g_name'];} 
if ($logged=="yes") {$gphone = $_SESSION[‘phone’];} else {$gphone = $_POST['g_phone'];}
if ($logged=="yes") {$gemail = $_SESSION[‘email’];} else {$gemail = $_POST['g_email'];}
$gmesg = $_POST['g_mesg'];
$gift = $_POST['gift'];
...
//------------Process 4--------------Checking retyped password and displaying message.
  elseif ($pass!=$pass2)
  {  
  	 echo  "<img src='images/speech4.png' border='0' usemap='#Map4' />    ";	
    $hasvalue = 0;
  } 
...
//---------Process 5-------------If everything is  ok a check to see if user id already taken and display message
if ($hasvalue==1) 
{
  ...
  while ($row = mysql_fetch_array($result)) 
  {
    ...
    if ($user_id==$u_id) //----------------Process 6------------ if the user id is already taken then...
    {   
		echo "<img src='images/speech5.png' border='0' usemap='#Map3' />";
		$write = 0;
		break;  
    }
...
//----------Process 7---------------If write variable is 1 then begin to Data writing to user details table and gift table
if ($write==1)
{
	$query = " INSERT INTO user_details ( u_id, pass, f_name, r_add1, r_add2, city, pincode, phone, email )  VALUES ('$user_id', '$pass', '$name', '$add1', '$add2', '$city', '$pin', '$phone', '$email') "; 
	$result = mysql_query($query, $db) or die(mysql_error($db)); 
}

if ($gift=="yes")
{
	$query = " INSERT INTO gift ( gname, gphone, gemail, gmesg, u_id)  VALUES ('$gname', '$gphone', '$gemail', '$gmesg', '$user_id') "; 
	$result = mysql_query($query, $db) or die(mysql_error($db)); 
}
